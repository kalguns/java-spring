...
public interface AccountRepository extends JpaRepository<Account,Long> {
   ...
   List<Account> fetchAllJPQL();
    ...
    @Query(value = "SELECT * FORM account_details WHERE age < ?1",nativeQuery = true)
    List<Account> retrieveAllByAgeLowerThan(Integer age);

    //Write a native query to read all accounts that a specific value can be containable in the name, address, country, state city
    @Query(value = "SELECT * FROM account_details WHERE name ILIKE concat('%',?1,'%') OR country ILIKE ILIKE concat('%',?1,'%') OR address ILIKE concat('%',?1,'%'), OR state ILIKE concat('%',?1,'%') ",nativeQuery = true)
