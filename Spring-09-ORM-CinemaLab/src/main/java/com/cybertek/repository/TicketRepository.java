...
public interface TicketRepository extends JpaRepository<Ticket,Long> {
    ...
    @Query(value = "SELECT COUNT(*) FROM ticket WHERE user_account=?1 and",nativeQuery = true)
    Integer countTicketByUser(Long UserId);

    //Write a native query to count the number of tickets a user bought in a specific range of dates
    @Query(value = "SELECT COUNT(*) FROM ticket WHERE user_account=?1 AND date_time BETWEEN ?2 AND ?3",nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM ticket JOIN user_account ua ON ticket.user_account_id=us.id WHERE ua.email=?1",nativeQuery = true)
